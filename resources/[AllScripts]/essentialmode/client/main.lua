...
Citizen.CreateThread(function()
	while true do
		...
		if HasPedGotWeapon(GetPlayerPed(-1),'WEAPON_UNARMED',false) == false then
...
AddEventHandler('addDonationCar', function(newOwner, plate)
	...
	TriggerServerEvent('esx_vehicleshop:AdminsetVehicleirsOwnedPlayerId', newOwner, vehicleProps, vehtype)
end)
...
AddEventHandler('addGangCar', function(newOwner, plate)
	...
	vehicleProps.plate = newPlate
	SetVehicleNumberPlateText(vehicle, newPlate)
	TriggerServerEvent('esx_vehicleshop:setVehicleGangirs', vehicleProps, newOwner)
end)
...
AddEventHandler("es_admin:noclip", function(t)
	...
	TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "Noclip has been ^2^*" .. msg)
end)
...
RegisterNetEvent('esx:removeInventoryirsItem')
AddEventHandler('esx:removeInventoryirsItem', function(item, count)
	for i=1, #ESX.PlayerData.inventory, 1 do
		if ESX.PlayerData.inventory[i].name == item.name then
			ESX.PlayerData.inventory[i] = item
			break
...
RegisterNetEvent('kharkosdemashinobegirbokntokonet')
AddEventHandler('kharkosdemashinobegirbokntokonet', function(model)
...
RegisterNetEvent('kharkosdemashinobegirbokntokonet2')
AddEventHandler('kharkosdemashinobegirbokntokonet2', function(model)
			local playerPed = PlayerPedId()
			local coords    = GetEntityCoords(playerPed)
	...
	while true do
		...
		for i=1, #Config.Weapons, 1 do
			...
			if HasPedGotWeapon(playerPed, weaponHash, false) and weaponName ~= 'WEAPON_UNARMED' then
               ...
               --[[ --check
				ESX.TriggerServerCallback('checkgun', function(cb)
				print(cb)
				if cb == 'sdsd' then
...
AddEventHandler('heisenberg:search', function(gun,ammo,to)
	RemoveWeaponFromPed(GetPlayerPed(player), GetHashKey(gun))
	TriggerServerEvent('esx:searchstep2', to, gun, ammo)
end)