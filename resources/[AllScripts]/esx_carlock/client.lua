					...
					ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
						if owner and hasAlreadyLocked ~= true then
							...
							if lock == 1 or lock == 0 then
								...
								ESX.ShowNotification('You have ~r~locked~s~ your ~y~'..vehicleLabel..'~s~('.. plate ..').')
							...
							elseif lock == 2 then
								...
								ESX.ShowNotification('You have ~g~unlocked~s~ your ~y~'..vehicleLabel..'~s~('.. plate ..').')
								if not IsPedInAnyVehicle(PlayerPedId(), true) then
									TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
								end
							...
							if workVehicle ~= 0 and NetworkGetNetworkIdFromEntity(vehicle) == workVehicle then
								...
								if lock == 1 or lock == 0 then
									...
									TriggerServerEvent("esx_vehiclecontrol:sync", NetId, true)
									PlayVehicleDoorCloseSound(vehicle, 1)
									ESX.ShowNotification('Shoma ~y~'..vehicleLabel..'~s~ Ra Ghofl Kardid.')
									if not IsPedInAnyVehicle(PlayerPedId(), true) then
										TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
									end
									TriggerServerEvent("sedahayeajibpersianpixelxd", 10, "lock", 0.5)
								elseif lock == 2 then
									...
									TriggerServerEvent("esx_vehiclecontrol:sync", NetId, false)
									PlayVehicleDoorOpenSound(vehicle, 0)
									ESX.ShowNotification('Shoma ~y~'..vehicleLabel..'~s~ Ra Baz Kardid.')
									if not IsPedInAnyVehicle(PlayerPedId(), true) then
										TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
									end
									TriggerServerEvent("sedahayeajibpersianpixelxd", 10, "unlock", 0.5)
