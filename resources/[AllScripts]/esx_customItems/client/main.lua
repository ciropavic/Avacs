...
RegisterCommand('deattach', function(source, args)
	...
	if IsPedArmed(ped, 4) then
		...
		if not args[1] then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma dar argument aval chizi vared nakardid!")
			return
		end
		...
		elseif input == "all" then
			...
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Argument vared shode eshtebah ast!")
		end
