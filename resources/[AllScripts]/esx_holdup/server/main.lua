		...
		if not rob then
			...
			--	local blowtorch = xPlayer.getInventoryItem("blowtorch")
				...
				--if blowtorch.count > 0 then
					...
					TriggerEvent('DiscordBot:ToDiscord', 'rob', name, oocname .. '('.. xPlayer.source ..') Started store robbery in ' ..store.nameofstore ,'user', true, source, false)
...
AddEventHandler('esx_holdup:robcomplete', function(robb)
	...
	TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Log", GetPlayerName(_source) .. '('.. xPlayer.source ..') have completed store robbery in ' .. store.nameofstore ,'user', true, source, false)
...
AddEventHandler('esx_holdup:robcancel', function(robb)
	...
	local award = store.reward
	local savedSource = _source
	TriggerEvent('DiscordBot:ToDiscord', 'rob', "Rob Log", GetPlayerName(_source) .. ' ('.. xPlayer.source ..') canceled store robbery in ' .. store.nameofstore ,'user', true, source, false)
