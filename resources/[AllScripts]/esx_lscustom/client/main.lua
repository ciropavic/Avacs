...
RegisterNetEvent('esx_lscustom:CloseMenus')
AddEventHandler('esx_lscustom:CloseMenus', function()
...
RegisterCommand('custom', function()
	...
	local inGarage = GetDistanceBetweenCoords(coords, -337.392578125, -138.95645141602, 39.009632110596, true) < 12
	local inGarage2 = GetDistanceBetweenCoords(coords, 1174.56, 2639.62 , 37.76, true) < 12

	if PlayerData.job ~= nil and PlayerData.job.name == 'mechanic' and PlayerData.job.grade > 2 then
		if inGarage or inGarage2  then
...
local CustomLocations = {
	{x = 1174.07,y = 2640.19,z = 37.77},
	{x = -338.38,y = -136.36,z = 39.01},

}
RegisterNetEvent('esx_lscustom:Default')
AddEventHandler('esx_lscustom:Default', function(vehicle)
	print(vehicle)
	NoChangeCar = vehicle
end)
				...
				ESX.TriggerServerCallback('esx_lscustom:checkStatus', function(ordered)
					...
					local playerPed = PlayerPedId()
	                 ...
	                 local PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z }
                       ...
                       TriggerServerEvent('esx_addons_gcphone:startCall', 'mechanic', 'Salam Man Mechanic Niaz Daram Dar Garage Montazer Shoma Hastam', PlayerCoords, {

		                PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
	                    })
						AlreadyCalledMechanic = true
						FreezeEntityPosition(vehicle, true)
						TriggerServerEvent('esx_lscustom:VehiclesInWatingList', DefaultCar.plate ,DefaultCar, true)
					elseif ordered then
									...
									if data.current.value == 'cash' then
										ESX.TriggerServerCallback('esx_lscustom:PayVehicleOrders', function(success)
											if success then
												...
												TriggerServerEvent('esx_lscustom:VehiclesInWatingList', DefaultCar.plate ,DefaultCar, false)
											else
