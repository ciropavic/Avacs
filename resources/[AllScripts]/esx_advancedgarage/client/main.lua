...
function ListOwnedCarsMenu()
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			ESX.ShowNotification("~r~Shoma Mashin Nadarid!")
			menu.close()
		else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				...
				ESX.ShowNotification("~r~In Khodro Dar Hale Hazer Dar Parking Nist!")
			end
		end, function(data, menu)
			menu.close()
		end)
...
RegisterNetEvent("ParkKonInoTooParking")
AddEventHandler("ParkKonInoTooParking",function()
	local playerPed    = GetPlayerPed(-1)
	if IsPedInAnyVehicle(playerPed,false) then
		local coords       = GetEntityCoords(playerPed)
		local vehicle      = GetVehiclePedIsIn(playerPed, false)
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			...
			ESX.ShowNotification("~w~Shoma In Khodro Ra Dar ~g~Parking~w~ Qarar Dadid!")
		end, vehicleProps)
	else
		ESX.ShowNotification("~w~Aval Savare Yek vasile naghlie Besho!")
	end
end)
...
function putaway(vehicle, vehicleProps)
	...
	TriggerEvent('chat:addMessage', {
		color = { 255, 0, 0},
		multiline = true,
		args = {" [ SYSTEM | ECRP ] ", "^0Shoma baraye estefade az in dastor bayad ranande bashid!"}
		})
...
function SpawnVehicle(vehicle, plate)
	if this_Garage.GName == 1 then
		local myChance = math.random(1,10)
		if myChance == 1 then
			nowX,nowY,nowZ,nowH = 235.15,-771.69,31.02,248.31
		elseif myChance == 2 then
			nowX,nowY,nowZ,nowH = 233.08,-776.86,30.99,249.24
		elseif myChance == 3 then
			nowX,nowY,nowZ,nowH = 231.23,-781.85,30.96,250.32
		elseif myChance == 4 then
			nowX,nowY,nowZ,nowH = 229.23,-786.81,30.96,249.36
		elseif myChance == 5 then
			nowX,nowY,nowZ,nowH = 227.38,-791.86,30.93,249.45
		elseif myChance == 6 then
			nowX,nowY,nowZ,nowH = 225.81,-796.94,30.92,249.32
		elseif myChance == 7 then
			nowX,nowY,nowZ,nowH = 224.13,-802.19,30.91,248.51
		elseif myChance == 8 then
			nowX,nowY,nowZ,nowH = 215.12,-801.34,31.09,69.29
		elseif myChance == 9 then
			nowX,nowY,nowZ,nowH = 220.23,-786.24,31.06,69.47
		elseif myChance == 10 then
			nowX,nowY,nowZ,nowH = 226.25,-771.1,31.06,69.24
		end
		ESX.Game.SpawnVehicle(vehicle.model, {
			x = nowX,
			y = nowY,
			z = nowZ + 1
		}, nowH, function(callback_vehicle)
		...
		if IsVehicleSeatFree(entity, -1) then
			if DoesEntityExist(entity) then
				TriggerEvent('chat:addMessage', {
					color = { 255, 0, 0},
					multiline = true,
					args = {" [ SYSTEM | ECRP ] ", "^2 " .. carName .. "^0 ba movafaghiat hazf shod!"}
				})
			end
