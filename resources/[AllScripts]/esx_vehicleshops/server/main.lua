...
AddEventHandler('esx_vehicleshop:setVehicle2Owned', function (vehicleProps, society)
	...
	if found and xPlayer.money >= vehicleData.price then
		...
		TriggerEvent('DiscordBot:ToDiscord', 'buycar',  xPlayer.name .. '('.. _source .. ')' , '```css\n' .. GetPlayerName(_source).. '('.. xPlayer.name .. ')\nYek Mashin('.. vehicleData.name ..') Ba Gheimat : '.. vehicleData.price ..'$ Kharid\n```')
...
RegisterServerEvent('esx_vehicleshop:addvehiclezeta')
AddEventHandler('esx_vehicleshop:addvehiclezeta', function (playerId, vehicleProps)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(playerId)

	local xPlayerSource = ESX.GetPlayerFromId(source)
   if xPlayerSource.permission_level >= 8 then
	for i = 1, #Vehicles, 1 do
        if GetHashKey(Vehicles[i].model) == vehicleProps.model then
            vehicleData = Vehicles[i]
			found = true
