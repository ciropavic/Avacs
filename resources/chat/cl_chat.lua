  ...
  RegisterNetEvent('_chat:messageEntshirmikhoriered')
  ...
  RegisterNUICallback('chatResult', function(data, cb)
    ...
    if not data.canceled then
      ...
      if data.message:sub(1, 1) == '/' then
        if not muted then
          ...
          if data.message:sub(1, 4) == '/ooc' or data.message:sub(1, 2) == '/b' or data.message:sub(1, 2) == '/s' or data.message:sub(1, 3) == '/mp' or data.message:sub(1, 3) == '/me' or data.message:sub(1, 4) == '/do' then
            ...
            TriggerEvent('chat:addMessage', {
              color = { 255, 0, 0},
              multiline = true,
              args = {" [ SYSTEM | PersianPixel ] ", "^0Shoma nemitavanid hengami ke ^1mute ^0hastid chat konid!"}
              })
        ...
        if not muted then
          TriggerServerEvent('_chat:messageEntshirmikhoriered', GetPlayerName(id), { r, g, b }, data.message)
        else
