...
local function Sell(source, zone)
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			if vine == 0 then
				...
				return
			elseif xPlayer.getInventoryItem('vine').count <= 0 then
				...
				if (vine == 1) then
					SetTimeout(4000, function()
						local money = math.random(4000,5000)
	...
	TriggerClientEvent('esx:showNotification', xPlayer.source, 'Shoma Nemitavanid Chizi Befroushid')
	end
	end
