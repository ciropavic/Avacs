...
AddEventHandler('esx:setJob', function(job)
 ...
 AddEventHandler('esx_ambulancejob:ReviveIfDead', function()
	...
	TriggerServerEvent('esx_ambulancejob:irssetDeathStatus', false)
...
RegisterCommand('cpr', function(source, args)
	if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "ambulance" then
		...
		if not args[1] then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma dar ghesmat ID chizi vared nakardid!")
			return
		end
		
		if not tonumber(args[1]) then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma dar ghesmat ID faghat mojaz be vared kardan adad hastid!")
			return
		end
		...
		if GetPlayerName(PlayerId()) == GetPlayerName(GetPlayerFromServerId(target)) then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid khodetan ra cpr konid!")
			return
		end

		if GetPlayerName(GetPlayerFromServerId(target)) == "**Invalid**" then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0ID vared shode eshtebah ast")
			return
		end
		...
		if GetDistanceBetweenCoords(coords, tcoords, true) > 2 then
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Faselel shoma az player mored nazar ziad ast")
			return
		end
			...
			if IsDead then
				...
				ESX.TriggerServerCallback('ambulance_job:getCprStatus', function(IsCpr)
					if not IsCpr then
						...
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Player mored nazar ghablan CPR shode ast!")
					end
				end, target)


			else
				TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid player zende ra cpr konid!")
			end
		end, target)	

	else
		TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma dastresi kafi baraye estefade az in dastor ra nadarid!")
	end
end, false)
...
RegisterCommand('drag', function(source, args)
	...
	if ESX.PlayerData.job.name == "ambulance" then
		...
		if busy == true and PedIsBeingCarried ~= nil then
			...
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma brankard ra vel kardid!")
			busy = false
	   ...
	   if args[1] then
			...
			if target then
				...
				if GetPlayerName(GetPlayerFromServerId(target)) ~= "**Invalid**" then
					...
					if GetPlayerName(PlayerId()) == GetPlayerName(GetPlayerFromServerId(target)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid khodetan ra drag koid!")
						return
					end
					...
					if GetDistanceBetweenCoords(coords, tcoords, true) < 2 then
						...
						ESX.TriggerServerCallback('ambulance_job:checkdeath', function(IsDead)
							if IsDead then
								...
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma ba movafaghiat ^1" .. GetPlayerName(GetPlayerFromServerId(target))  .. "^0 ra roye brankard gozashtid")

							else
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid player zende ra roye brankard gharar dahid!")
							end
						end, target)

					else
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma az player mored nazar khili door hastid!")
					end

				else
					TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Player mored nazar online nist!")
				end

			else
				TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma dar ghesmat ID faghat mitavanid adad vared konid!")
			end
		...
		TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma medic nistid!")
	end
end, false)
...
RegisterCommand('loadpt', function(source, args)
	if ESX.PlayerData.job.name == "ambulance" then
		...
		if busy == true and PedIsBeingCarried ~= nil or isDragging then
			...
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitvaniid dar hale hamle brankard bimar ra savar konid!")
			return
	   ...
	   if args[1] then
			...
			if target then
				...
				if GetPlayerName(GetPlayerFromServerId(target)) ~= "**Invalid**" then
					...
					if GetPlayerName(PlayerId()) == GetPlayerName(GetPlayerFromServerId(target)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid khodetan ra vared ambulance konid")
						return
					end

					if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitvanaid hengami ke dakhel mashin hastid az command estefade konid")
						return
					end
                    ...
                    if vehhash == 1171614426 or vehhash ==  -974922913 then
					if GetDistanceBetweenCoords(pcoords, tcoords, true) < 5 then
						...
						ESX.TriggerServerCallback('ambulance_job:checkdeath', function(IsDead)
							if IsDead then
								...
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma ba movafaghiat ^1" .. GetPlayerName(GetPlayerFromServerId(target))  .. "^0 ra vared ambulance kardid")

							else
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid player zende ra daron ambulance gharar dahid!")
							end
						end, target)

					else
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Player mored nazar az ambulance khili door ast!")
					end
					else
					TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0 Mashin Ambulance Peyda Nashod!")
					end
...
RegisterCommand('stabilize', function(source, args)
	if ESX.PlayerData.job.name == "ambulance" then
		...
		if busy == true and PedIsBeingCarried ~= nil or isDragging then
			...
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitvaniid dar hale hamle brankard bimar ra stabilize konid!")
			return
	   ...
	   if args[1] then
			...
			if target then
				...
				if GetPlayerName(GetPlayerFromServerId(target)) ~= "**Invalid**" then
					...
					if GetPlayerName(PlayerId()) == GetPlayerName(GetPlayerFromServerId(target)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid khodetan ra stabilize konid")
						return
					end
								...
								if IsDead then
									...
									TriggerEvent("mythic_progbar:client:progress", {
										...
										if not status then
											...
											TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma ba movafaghiat ^1" .. GetPlayerName(GetPlayerFromServerId(target))  .. "^0 ra stabilize kardid!")

										end
									end)

								else
									TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid player zende ra stabilize konid!")
								end
							end, target)
							
					else
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Player mored nazar khili door ast!")
					end
...
RegisterCommand('deliverpt', function(source, args)
	if ESX.PlayerData.job.name == "ambulance" then
		...
		if busy == true and PedIsBeingCarried ~= nil or isDragging then
			...
			TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitvaniid dar hale hamle brankard bimar ra tahvil dadid!")
			return
	   ...
	   if args[1] then
			...
			if target then
				...
				if GetPlayerName(GetPlayerFromServerId(target)) ~= "**Invalid**" then
					...
					if GetPlayerName(PlayerId()) == GetPlayerName(GetPlayerFromServerId(target)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid khodetan ra darman konid")
						return
					end

					if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma baraye estefade az in command bayad dakhel mashin bashid")
						return
					end
					...
					if checkDistance() then
						...
						ESX.TriggerServerCallback('ambulance_job:checkdeath', function(IsDead)
							if IsDead then
								...
								TriggerServerEvent('esx_ambulancejob:irsrevive', target)
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma ba movafaghiat ^1" .. GetPlayerName(GetPlayerFromServerId(target))  .. "^0 ra darman kardid")

							else
								TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Shoma nemitavanid player zende ra darman konid!")
								return
							end
						end, target)

					else
						TriggerEvent("chatMessage", " [ SYSTEM | PersianPixel ] ", {255, 0, 0}, "^0Bimar nazdik bimarestan nist!")
					end
...
function OnPlayerDeath()
	if injured == false then
		...
		RespawnPed(playerPed, formattedCoords, 0.0)
		injured = true
		TriggerServerEvent('esx_ambulancejob:irssetDeathStatus', true)
		TriggerServerEvent('esx_ambulancejob:chagneCprStatus', GetPlayerServerId(PlayerId()), false)
...
RegisterNetEvent('esx_ambulancejob:irsrevive')
AddEventHandler('esx_ambulancejob:irsrevive', function()
	local playerPed = PlayerPedId()
	local coords = GetEntityCoords(playerPed)
	IsDead = false
	injured = false
