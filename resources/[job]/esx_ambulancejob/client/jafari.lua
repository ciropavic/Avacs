...
Configjafari.Price = 1000 -- Edit this to your liking.
...
Configjafari.Locations = {
	{ x = 451.52, y = -980.45, z = 25.7, heading = 229.86 },
	{ x = 333.58, y = -579.44, z = 42.3 ,heading = 161.89 },
}
...
Citizen.CreateThread(function()
	while true do
		...
		for k,v in pairs(Configjafari.Zones) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Configjafari.DrawDistance) then
				canSleep = false
...
Citizen.CreateThread(function()
	...
	if Configjafari.EnablePeds then
		for _, item in pairs(Configjafari.Locations) do
