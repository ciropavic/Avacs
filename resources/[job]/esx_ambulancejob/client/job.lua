...
function OpenAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		...
		elseif data.current.value == 'boss_actions' then
			...
			TriggerEvent('esx_society:openbossirs', 'ambulance', function(data, menu)
						 	...
						 	if quantity > 0 then
						 		...
						 		ESX.TriggerServerCallback('ambulance_job:checkdeath', function(IsDead)
						 			if IsDead then
						 				...
						 				TriggerServerEvent('makoniiimkhobvalikonmikonim', PedToNet(GetPlayerPed(-1)),GetPlayerServerId(closestPlayer))
						 				...
						 				TriggerEvent("mythic_progbar:client:progress", {
						 					...
						 					if not status then
						 						...
						 						TriggerServerEvent('esx_ambulancejob:irsrevive', GetPlayerServerId(closestPlayer))
						-- 						-- Show revive award?
		...
		elseif part == 'VehiclesDeleter' then
			...
			if IsPedInAnyVehicle(GetPlayerPed(-1),  false) then
				...
				if DoesEntityExist(vehicle) then
				  ...
				  CurrentActionMsg  = 'Dokme ~INPUT_CONTEXT~ Jahat Hazf Mashin!'
			...
			if IsControlJustReleased(0, Keys['E']) then
				...
				elseif CurrentAction == 'VehiclesDeleter' then
					-- print(CurrentActionData.vehicle)
					 ESX.Game.DeleteVehicle(GetVehiclePedIsUsing(GetPlayerPed(-1)))
					...
					local playerPed = PlayerPedId()
						...
						if foundSpawn then
							...
							--	TriggerServerEvent("esx_policejob:addVehicle", callsign, NetId)
                                ...
                                SetVehicleNumberPlateText(vehicle, "EMS " .. output)
								Citizen.CreateThread(function()
			                 	Citizen.Wait(2000)
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		TriggerServerEvent('esx_ambulancejob:giveItemirs', data.current.value)
