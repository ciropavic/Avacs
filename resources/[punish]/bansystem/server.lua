...
TriggerEvent('es:addAdminCommand', 'unban', 11, function (source, args, user)
	if args[1] and string.find(args[1], "steam:") == 1 then
		...
		MySQL.Async.fetchAll('SELECT * FROM banlist WHERE identifier = @steamHex',
		{
			['@steamHex'] = steamHex
		}, function(result)
			if result ~= nil then
...
TriggerEvent('es:addAdminCommand', 'ban', 7, function (source, args, user)
 ...
 if ESX.GetPlayerFromId(target).permission_level > xPlayer.permission_level then TriggerClientEvent('chat:addMessage', source, { args = { '^1Ban System :', '^2 Admin Rank Balataro Ban Mikoni? Bayad Bokhorish :|' } }) return end
	if args[1] then		
		if reason == "" then
			reason = 'Shoma Az Server Ban Shodid Lotfa Be Discord Server Moraje Konid , Taskhor | dicord.gg/persianpixel'
		end
		...
		if time then
			if target and string.find(target, "steam:") == 1 then
				for i = 1, #BanList, 1 do
					if BanList[i].identifier == target then
						TriggerClientEvent('chat:addMessage', source, { args = { ' ^1Ban ', ' In Player Dar Hale Hazer Ban Mibashad! Name: '.. BanList[i].name } })
						return
					end
				end
...
function bancheater(source,reason)
	...
	for k,v in ipairs(GetPlayerIdentifiers(_source))do
		...
		"Discord UID: " .. discord:gsub('discord:', '') .. "\n");
		--sendToDisc("wtf")
		TriggerClientEvent('chatMessage', -1, "[ PersianPixel AC ]", {255, 0, 0}, "^1" .. targetplayername .. " ^0Tavasot ^2 " .. sourceplayername .. " ^0Ban Shod [ Permanent Ban ] Be Dalile: ^3" .. reason )
		...
		DropPlayer(_source, ' 🤧🤧 Shoma Ban Dost Geramimi Cheara ? Be Dalil Cheate : [ '.. reason ..' ] Onam Tavasot Anticheat Gogoli Ban Shodi 😄😄')
		end
		else
    ...
    embed = {
        {
            ...
            ["title"] = "Admin Bala Hichi Vaght Ban Nmishe",
            ["description"] = "[ Chi Behet Begam Ddsh | Koskholi ? ]",
            ["footer"] = {
                ["text"] = footer,
            },
        }
...
function anticheatsendToDiscLOG(footer)
    ...
    embed = {
        {
            ["color"] = 16711680, -- GREEN = 65280 --- RED = 16711680
            ["title"] = " Cheate Bega Raft ",
            ["description"] = "[ Bye Bye My Little Poosy ]",
            ["footer"] = {
                ["text"] = footer,
            },
        }
