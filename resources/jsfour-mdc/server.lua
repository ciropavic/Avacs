...
ESX.RegisterServerCallback('jsfour-mdc:save', function(source, cb, data)
  MySQL.Async.fetchAll('SELECT playerName FROM users WHERE lastdigits = @lastdigits', {['@lastdigits'] = data.signedInUser},
  function (result)
    if result[1] ~= nil then
      local uploader  = result[1].playerName
...
ESX.RegisterServerCallback('jsfour-mdc:fetch', function(source, cb, data)
      ...
      if result[1] ~= nil then
        for i=1, #result, 1 do
          if string.upper(result[i]['plate']) == data.plate then
            ...
            MySQL.Async.fetchAll('SELECT playerName, dateofbirth, lastdigits FROM users WHERE identifier = @identifier', {['@identifier'] = identifier},
            function (result)
              if result[1] ~= nil then
                result = result
  ...
  elseif data.type == 'person' then
    ...
    MySQL.Async.fetchAll('SELECT identifier, playerName, dateofbirth, height, gender, phone_number FROM users WHERE lastdigits = @lastdigits', {['@lastdigits'] = data.lastdigits},
    function (result)
      if result[1] ~= nil then
        result = result
...
ESX.RegisterServerCallback('jsfour-mdc:remove', function(source, cb, data)
  ...
  MySQL.Async.fetchAll('SELECT playerName FROM users WHERE lastdigits = @lastdigits', {['@lastdigits'] = data.signedin},
  function (result)
    local remover = result[1].playerName
    local wanted  = nil
