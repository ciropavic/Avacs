...
AddEventHandler("esx_trunk:getItem",function(plate, type, item, count, max, owned)
              ...
              if coffre[i].name == item then
                if (coffre[i].count >= count and count > 0) then
                  ...
                  --local info = {source = xPlayer.source, icname = xPlayer.name, name = item, type = "Bardasht", plate = plate, count = "1"}
				   TriggerEvent('DiscordBot:ToDiscord', 'inventorytrunkbardashtanitem', GetPlayerName(xPlayer.source), '```fix\n' .. GetPlayerName(xPlayer.source) .. '('.. xPlayer.source ..')\n'.. xPlayer.name ..'\nsteam : '.. xPlayer.identifier ..'\n'..  item ..'('.. count ..')\nPlate (' .. plate .. ')\n```')
    ...
    if type == "item_weapon" then
      TriggerEvent("esx_trunk:getSharedDataStore", plate, function(store)
         ...
         -- exports.sniperlog:TrunkLog(info)
          TriggerEvent('DiscordBot:ToDiscord', 'inventorytrunkbardashtangun', GetPlayerName(xPlayer.source), '```fix\n' .. GetPlayerName(xPlayer.source) .. '('.. xPlayer.source ..')\n'.. xPlayer.name ..'\nsteam : '.. xPlayer.identifier ..'\n'.. weaponName ..'\nPlate (' .. plate .. ')\n```')
          local blackMoney = 0
          local items = {}
          local weapons = {}
          weapons = (store.get("weapons") or {})
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item) * count)) > max then
              ...
              if xPlayer.getInventoryItem(item).count > 0 then
                ...
                local info = {source = xPlayer.source, icname = xPlayer.name, name = item, type = "Gozasht", plate = plate, count = count}
				 TriggerEvent('DiscordBot:ToDiscord', 'inventorytrunkgozashtanitem', GetPlayerName(xPlayer.source), '```fix\n' .. GetPlayerName(xPlayer.source) .. '('.. xPlayer.source ..')\n'.. xPlayer.name ..'\nsteam : '.. xPlayer.identifier ..'\n'..  item ..'('.. count ..')\nPlate (' .. plate .. ')\n```')
               -- exports.sniperlog:TrunkLog(info)
              end
            ...
            if (getTotalInventoryWeight(plate) + (getItemWeight(item))) > max then
              ...
              store.set("weapons", storeWeapons)
                ...
                MySQL.Async.execute(
				  ...
				  TriggerEvent('DiscordBot:ToDiscord', 'inventorytrunkgozashtangun', GetPlayerName(xPlayer.source), '```fix\n' .. GetPlayerName(xPlayer.source) .. '('.. xPlayer.source ..')\n'.. xPlayer.name ..'\nsteam : '.. xPlayer.identifier ..'\n'..  item ..'\nPlate (' .. plate .. ')\n```')
                 -- exports.sniperlog:TrunkLog(info)

            end
          end)
      else
	  TriggerEvent('DiscordBot:ToDiscord', 'inventorytrundup', GetPlayerName(xPlayer.source), '```fix\n' .. GetPlayerName(xPlayer.source) .. '('.. xPlayer.source ..')\n'.. xPlayer.name ..'\nsteam : '.. xPlayer.identifier ..'\nTried to duplicate guns in the trunk\n```')
    --    exports.BanSql:BanTarget(xPlayer.source, "Tried to duplicate guns in the trunk", "Duplicate Weapons")
	exports.PERSIANPIXEL_AC.adminmsg(xPlayer.source,"Try To Duplicate :"..item)
      end
