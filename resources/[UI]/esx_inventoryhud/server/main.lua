...
RegisterCommand(
	...
	function(source, args, rawCommand)
		...
		if sourceXPlayer.permission_level > 5 then
			local target = tonumber(args[1])
			local targetXPlayer = ESX.GetPlayerFromId(target)

			if targetXPlayer ~= nil then
				TriggerClientEvent("esx_inventoryhud:openPlayerInventoryirs", source, target, targetXPlayer.name)
			else
