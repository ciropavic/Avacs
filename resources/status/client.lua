...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  if string.lower(job.name) ~= 'nojob' and (string.lower(job.name) ~= 'police' or job.ext == nil) then
    if job.grade > 0 then
      SendNUIMessage({action = "job", value = job.label .. " | " .. job.grade_label .. " | " .. job.grade, icon = job.name})
    else
      SendNUIMessage({action = "job", value = job.label .. " | " .. 'Off-Duty' .. " | " .. job.grade, icon = job.name})
    end    
  elseif job.ext and job.name == 'police' then
    if job.grade > 0 then
      SendNUIMessage({action = "job", value = job.ext:gsub("^%l", string.upper) .. " | " .. job.grade_label .. " | " .. job.grade, icon = job.ext})
    else
...
AddEventHandler('esx:setJob', function(job)
  ...
  elseif job.ext and job.name == 'police' then
    if job.grade > 0 then
      ...
      SendNUIMessage({action = "job", value = job.ext:gsub("^%l", string.upper) .. " | " .. 'Off-Duty' .. " | " .. job.grade, icon = job.ext})
    end    
  else
