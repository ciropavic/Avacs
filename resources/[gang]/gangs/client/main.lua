...
function OpenBossMenu(gang, close, options)
	...
	table.insert(elements, {label = 'Set Armory Access', value = 'set_armoryperm'})
	table.insert(elements, {label = 'Set Hud Icon', value = 'set_icon'})
	table.insert(elements, {label = 'Set Invite Access', value = 'set_inv'})
	if ESX.GetPlayerData()['CanGangLog'] == 1 then
	table.insert(elements, {label = "Set Log Web Hook", value = 'set_webhook'})
    end
 	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. gang, {
		title    = _U('boss_menu'),
		align    = 'top-right',
		...
		elseif data.current.value == 'set_webhook' then
			SetWebhook()
		elseif data.current.value == 'set_armoryperm' then
			SetArmoryAccess()
		elseif data.current.value == 'set_icon' then
			SetIcon()
		elseif data.current.value == 'set_inv' then
			SetInv()
		end
...
function OpenManageEmployeesMenu(gang)
 	...
 	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_employees_' .. gang, {
		...
		elements = {
			...
			--{label = "Set Log Web Hook", value = 'set_webhook'}
		}
	}, function(data, menu)
...
function SetArmoryAccess()
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_perm', {
				...
				ESX.TriggerServerCallback('gangs:setarmoryperm', function(refresh)
						menu2.close()
