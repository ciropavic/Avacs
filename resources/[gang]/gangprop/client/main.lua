...
if Config.EnableArmoryManagement then
  ...
  local elements = {
    {label = 'Anbar Gang', value = 'property_inventory'},
  }
  ...
  if data.current.value == "property_inventory" then
    if PlayerData.gang.grade >= Data.armoryperm then
      menu.close()
...
--[[Citizen.CreateThread(function()
    while true do
      Citizen.Wait(10)
		...
		if IsControlPressed(0, Keys["F5"])  and PlayerData.gang ~= nil and PlayerData.gang.label == 'gang' then
		 local temp = ESX.GetPlayerData()
             if temp.IsDead ~= 1 and temp.HandCuffed ~= 1 and temp.robbing ~= 1 and temp.jailed ~= 1 then
              OpenGangActionsMenu()
             end
...
AddEventHandler('gangprop:updateBlip', function()
	...
	-- Refresh all blips
	--[[for k, existingBlip in pairs(blipsGangs) do
