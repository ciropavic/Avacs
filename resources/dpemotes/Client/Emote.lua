...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlPressed(0, 73) then local data = ESX.GetPlayerData() if data.IsDead ~= 1 and data.HandCuffed ~= 1 then print(data.IsDead) EmoteCancel() ClearPedTasks(GetPlayerPed(-1)) end end
    Citizen.Wait(1)
  end
end)
...
RegisterNetEvent('dp:changestatus')
  AddEventHandler('dp:changestatus', function(chi)
	  cancommand = chi
  end)
